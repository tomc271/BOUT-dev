...
class GEM : public PhysicsModel {
  ...
  SpatialDimensions::FieldMetric Grad_par_logB; // Grad_par(log(B))
  ...
  void calc_aux() {
    ...
    if (small_rho_e) {
      ...
      Field3D dn = gyroPade1(Ne, rho_e) + gyroPade2(Teperp, rho_e) - gyroPade1(Ni, rho_i)
                   - gyroPade2(Tiperp, rho_i);
  ...
  int convective(BoutReal UNUSED(time)) override {
    ...
    if (!adiabatic_electrons) {
      ...
      if (tepar_ddt) {
        ...
        if (nonlinear) {
          ddt(Tepar) +=
              -UE_Grad(Te0 + Tepar, phi_G) - 2. * Div_parP(Ue + qepar, CELL_CENTRE);
        } else {
          ddt(Tepar) += -UE_Grad(Te0, phi_G) - 2. * Div_par(Ue + qepar, CELL_CENTRE);
        }
      ...
      if (teperp_ddt) {
        ddt(Teperp) = +0.5
                          * curvature(phi_G + Phi_G + tau_e * (Ne + Teperp)
      ...
      if (qepar_ddt) {
        ddt(qepar) = -1.5 * (1. / mu_e) * Grad_parP(tau_e * Te0, CELL_YLOW)
        ...
        if (nonlinear) {
          ...
          ddt(qepar) += -1.5 * (1. / mu_e) * Grad_par(tau_e * Tepar, CELL_YLOW);
        }
      ...
      if (qeperp_ddt) {
        ...
        if (nonlinear) {
          ...
          ddt(qeperp) += -(1. / mu_e) * Grad_par(Phi_G + tau_e * Teperp, CELL_YLOW);
        }
    ...
    if (tipar_ddt) {
      ...
      if (nonlinear) {
        ddt(Tipar) +=
            -UE_Grad(Ti0 + Tipar, phi_G) - 2. * Div_parP(Ui + qipar, CELL_CENTRE);
      } else {
        ddt(Tipar) += -UE_Grad(Ti0, phi_G) - 2. * Div_par(Ui + qipar, CELL_CENTRE);
      }
    ...
    if (tiperp_ddt) {
      ddt(Tiperp) = +0.5
                        * curvature(phi_G + Phi_G + tau_i * (Ni + Tiperp)
      ...
      if (nonlinear) {
        ...
        ddt(Tiperp) += -UE_Grad(Ti0, phi_G) - WE_Grad(Ni0 + 2. * Ti0, Phi_G)
    ...
    if (qipar_ddt) {
      ...
      if (nonlinear) {
        ...
        ddt(qipar) += -1.5 * (1. / mu_i) * Grad_par(tau_i * Tipar, CELL_YLOW);
      }
    ...
    if (qiperp_ddt) {
      ...
      if (nonlinear) {
        ...
        ddt(qiperp) += -(1. / mu_i) * Grad_par(Phi_G + tau_i * Tiperp, CELL_YLOW);
      }
