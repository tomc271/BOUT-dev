...
macro(bout_handle_requires_conflicts TYPENAME TYPEVAR)
  ...
  foreach(REQUIREMENT IN LISTS BOUT_HANDLE_OPTIONS_REQUIRES)
    if(NOT ${REQUIREMENT})
      message(
        STATUS
          "Not building ${TYPENAME} ${TYPEVAR}, requirement not met: ${REQUIREMENT}"
      )
      return()
    endif()
  endforeach()
  ...
  foreach(CONFLICT IN LISTS BOUT_HANDLE_OPTIONS_CONFLICTS)
    if(${CONFLICT})
      message(
        STATUS
          "Not building ${TYPENAME} ${TYPEVAR}, conflicts with: ${CONFLICT}"
      )
      return()
    endif()
  endforeach()
...
function(bout_add_model MODEL)
  ...
  if(NOT BOUT_MODEL_OPTIONS_SOURCES)
    message(
      FATAL_ERROR "Required argument SOURCES missing from 'bout_add_model'"
    )
  endif()
...
function(bout_add_integrated_or_mms_test BUILD_CHECK_TARGET TESTNAME)
  ...
  cmake_parse_arguments(
    BOUT_TEST_OPTIONS "${options}" "${oneValueArgs}" "${multiValueArgs}"
    ${ARGN}
  )
  ...
  if(BOUT_TEST_OPTIONS_SOURCES)
    ...
    if(BOUT_TEST_OPTIONS_EXECUTABLE_NAME)
      set_target_properties(
        ${TESTNAME} PROPERTIES OUTPUT_NAME ${BOUT_TEST_OPTIONS_EXECUTABLE_NAME}
      )
    else()
      ...
      get_filename_component(
        BOUT_TEST_EXECUTABLE_NAME ${BOUT_TEST_FIRST_SOURCE} NAME_WE
      )
      set_target_properties(
        ${TESTNAME} PROPERTIES OUTPUT_NAME ${BOUT_TEST_EXECUTABLE_NAME}
      )
    endif()
  ...
  if(BOUT_TEST_OPTIONS_DOWNLOAD)
    ...
    add_custom_target(
      download_test_data DEPENDS ${BOUT_TEST_OPTIONS_DOWNLOAD_NAME}
    )
...
function(bout_add_library_alias dst src)
  ...
  foreach(name INTERFACE_LINK_LIBRARIES INTERFACE_INCLUDE_DIRECTORIES
