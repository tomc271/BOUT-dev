...
LaplacePCR_THOMAS::LaplacePCR_THOMAS(Options* opt, CELL_LOC loc, Mesh* mesh_in)
  ...
  if (!is_pow2(nxpe)) {
    throw BoutException("LaplacePCR_THOMAS error: NXPE must be a power of 2");
  }

  // Number of x points must be a power of 2
  if (!is_pow2(localmesh->GlobalNxNoBoundaries)) {
    throw BoutException("LaplacePCR_THOMAS error: GlobalNx must be a power of 2");
  }
...
FieldPerp LaplacePCR_THOMAS::solve(const FieldPerp& rhs, const FieldPerp& x0) {
 ...
 */
void LaplacePCR_THOMAS ::apply_boundary_conditions(const Matrix<dcomplex>& a,
...
void LaplacePCR_THOMAS ::pcr_forward_single_row(Matrix<dcomplex>& a, Matrix<dcomplex>& b,
  ...
  auto* sreq = new MPI_Request[2];
  auto* rreq = new MPI_Request[2];
  MPI_Comm comm = BoutComm::get();
 ...
 */
void LaplacePCR_THOMAS ::verify_solution(const Matrix<dcomplex>& a_ver,
