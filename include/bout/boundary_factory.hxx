...
public:
  ...
  BoundaryOpBase* create(std::string_view name, BoundaryRegionBase* region);

  /// Create a boundary object using the options file
  BoundaryOpBase* createFromOptions(std::string_view varname, BoundaryRegionBase* region);

  /*!
   * Add available boundary conditions and modifiers
   * Supply an object, and the name to be used
   */
  void add(BoundaryOp* bop, std::string_view name);

  /*!
   * Add a boundary condition modifier
   */
  void addMod(BoundaryModifier* bmod, std::string_view name);

  /*!
   * Note: This method should be removed, as the string method is sufficient
   */
  void add(BoundaryOpPar* bop, std::string_view name);
...
private:
  ...
  BoundaryOp* findBoundaryOp(std::string_view s);
  BoundaryModifier* findBoundaryMod(std::string_view s);
  // Parallel boundary conditions
  BoundaryOpPar* findBoundaryOpPar(std::string_view s);
  // To be implemented...
